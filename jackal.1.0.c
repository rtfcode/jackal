...
void print_cert(X509 *cert)
{
	...
	algorithm = OPENSSL_malloc(ALGO_LEN);
	pubalgo = OPENSSL_malloc(ALGO_LEN);
	seriallength = i2c_ASN1_INTEGER(X509_get_serialNumber(cert), NULL);
	serialnumber = OPENSSL_malloc(seriallength);
	hashlength = i2c_ASN1_BIT_STRING(cert->signature, NULL);
	hash = OPENSSL_malloc(hashlength);

	if ((!algorithm) || (!pubalgo) || (!hash)) {
		printf("print_cert malloc error\n");
		exit(1);
	}

	if (!i2t_ASN1_OBJECT(algorithm, ALGO_LEN, cert->sig_alg->algorithm)) {
		printf("print_cert i2t_ASN1_OBJECT failed\n");
		exit(1);
	}

	if (!i2t_ASN1_OBJECT(pubalgo, ALGO_LEN, cert->cert_info->key->algor->algorithm)) {
		printf("print_cert i2t_ASN1_OBJECT failed\n");
		exit(1);
	}

	certkey = X509_get_pubkey(cert);
	modulus = BN_bn2hex(certkey->pkey.rsa->n);

	temp = hash;
	if (!i2c_ASN1_BIT_STRING(cert->signature, &temp)) {
		printf("print_cert i2c_ASN1_BIT_STRING failed\n");
		exit(1);
	}

	temp = serialnumber;
	if (!i2c_ASN1_INTEGER(X509_get_serialNumber(cert), &temp)) {
		printf("print_cert i2c_ASN1_INTEGER failed\n");
		exit(1);
	}
	...
	hexdump(serialnumber, seriallength, 3);
...
int get_signing_algo(X509 *crt)
{
	...
	nid = OBJ_obj2nid(crt->sig_alg->algorithm);
	if (!nid) {
...
char *get_public_algo(X509 *crt)
{
	char *pubalgo = OPENSSL_malloc(ALGO_LEN);
	...
	if (!i2t_ASN1_OBJECT(pubalgo, ALGO_LEN, crt->cert_info->key->algor->algorithm)) {
		printf("i2d_ASN1_OBJECT failed\n");
		exit(1);
	}
...
EVP_PKEY *gen_new_key(int certkeysize)
{
	...
	newrsa = RSA_generate_key(certkeysize, 65537, 0, 0);
	if (!newrsa) {
		printf("gen_new_key RSA_generate_key failed\n");
		exit(1);
	}
	newkey = EVP_PKEY_new();
	if (!newkey) {
